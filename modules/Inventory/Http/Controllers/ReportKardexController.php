...
class ReportKardexController extends Controller{
    ...
    public function filter(){
        $items = Item::query()->whereNotIsSet()->where([['item_type_id', '01'], ['unit_type_id', '!=', 'ZZ']])->latest()->get()->transform(function ($row) {
    ...
    private function data($item_id, $date_start, $date_end){
        ...
        $data = InventoryKardex::with(['inventory_kardexable'])->where('warehouse_id', $warehouse->id);
        if ($date_start) {
    ...
    private function data3($item_id, $date_start, $date_end){
        if ($date_start && $date_end){
            $data = ItemLot::whereBetween('date', [$date_start, $date_end])->orderBy('item_id')->orderBy('id');
        } else {
